...
let shuffleIndex
let timeDuration = [500, 4500, 4000, 3500, 3000, 2500]
let timeIndex
...
const timerBar = () => {
    ...
    hint.className = "hint";
    hint.innerHTML = "<i class=\"fa-solid fa-question\"></i>"; // Question mark icon
    ...
    let hint_label = document.createElement("div");
    hint_label.className = "hint_label";
...
function getRandomImages() {
    ...
    for (let i = 0; i < totalCells /3 + 2 ; i++) {//totalCell have 12*12 but only need 100 pokemon each coner have 1 addition backgroud cell
...
const initVisited = () => {
    visited = Array.from({ length: gameWidth }, () => Array(gameHeight).fill(false));
};
...
const pauseButton = () => {
    ...
    if (!interval_status) {
        ...
        let text = document.getElementsByClassName("pause_label");
        ...
        countDown = setInterval(() => {
            ...
            countdown_value = countDown; // Move this line inside the else block
...
//click function
function mouseClicked(x, y, event) {
    if (countDown === null) {
        countDown = setInterval(() => {
            interval--;
            const progressWidth = interval - (interval * 0.0000000025);
            if (interval > 0) {
                timer.style.width = progressWidth + "%";
                interval_update = progressWidth;
            } else {
                clearInterval(countDown);
                timer.style.width = "0%";
                const timeoutAlert = `<div class="time_out">
                                        <p>TIME OUT!</p>
                                     </div>`;
                gameFrame.innerHTML += timeoutAlert;
            }
        }, timeIndex);
    }
    ...
    if (cells[x][y] !== 0) {
        ...
        if (clickedCell !== cell1) {
            if (!cell1) {
                div1 = clickedCell;
                cell1 = [x, y];
                div1.style.opacity = "0.5"; // Highlight the first cell
                ...
                finishPath = false;
                initVisited();
                const firstCellType = cells[cell1[0]][cell1[1]];
                const secondCellType = cells[cell2[0]][cell2[1]];
                if (firstCellType === secondCellType) {
                    pathWay(cell1, cell2);
                }
                if (!DONE) {
...
const pathWay = (cell1, cell2) => {
    ...
    while (stack.length !== 0) {
        ...
        // a cell surround by 4 cell that call neightbor and have cordinate below
        const neighbors = [
